...
startdate = '20201213'
enddate = '20201220'
readFile = open('%s/data/OxCGRT_latest.csv'%outfile_path,'r')
sepfile = readFile.read().split('\n')
prv_results=[]
wk_results=[]

for a in range (0,len(Countries)):
    for b in range (1,len(sepfile)-1):
    ...
    print(CaseM1,Case0,Case1)
    if Case0==0:
        prv_results.append(0)
    else:
...
readFile = open('%s/CovModel_modfile_1608003470.txt'%outfile_path,'r')
sepfile = readFile.read().split('\n')
prv_predicted=[]
prv_rng=[]
...
print(len(prv_predicted),len(prv_results),'should match')
...
print(np.sum(prv_hits),len(prv_hits))
