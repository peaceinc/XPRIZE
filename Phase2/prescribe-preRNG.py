...
PRESCRIPTORS_FILE = '/home/xprize/work/neat-checkpoint-17'
...
def prescribe(start_date_str: str,
    ...
    for prescription_idx, prescriptor in enumerate(prescriptors):
        ...
        while action_start_date <= end_date:
            ...
            ixx = -1*236*ACTION_DURATION

            cov = pred_df['PredictedDailyNewCases'].iloc[ixx:].sum()


            n1 = pres_df['C1_School closing'].iloc[ixx:].sum()
            n2 = pres_df['C2_Workplace closing'].iloc[ixx:].sum()
            n3 = pres_df['C3_Cancel public events'].iloc[ixx:].sum()
            n4 = pres_df['C4_Restrictions on gatherings'].iloc[ixx:].sum()
            n5 = pres_df['C5_Close public transport'].iloc[ixx:].sum()
            n6 = pres_df['C6_Stay at home requirements'].iloc[ixx:].sum()
            n7 = pres_df['C7_Restrictions on internal movement'].iloc[ixx:].sum()
            n8 = pres_df['C8_International travel controls'].iloc[ixx:].sum()
            n9 = pres_df['H1_Public information campaigns'].iloc[ixx:].sum()
            n10 = pres_df['H2_Testing policy'].iloc[ixx:].sum()
            n11 = pres_df['H3_Contact tracing'].iloc[ixx:].sum()
            n12 = pres_df['H6_Facial Coverings'].iloc[ixx:].sum()


            coolness = cov*(n1+n2+n3+n4+n5+n6+n7+n8+n9+n10+n11+n12)
            print(coolness)
            
            pred_df.to_csv('/home/xprize/work/prescriptions/pred_%d.csv'%dfCt)
            pres_df.to_csv('/home/xprize/work/prescriptions/pres_%d.csv'%dfCt)
            dfCt += 1
