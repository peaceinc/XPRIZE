...
PRESCRIPTORS_FILE = '/home/xprize/work/neat-checkpoint-5'
...
def prescribe(start_date_str: str,
        ...
        while action_start_date <= end_date:
            ...
            for a in range (0,len(categories)):
                for b in range (len(pert_df)+ixx,len(pert_df)):
                    ...
                    pert = HALO(cell,categories_max[a],HaloCallNum,HaloCellNum)
                    if pert!=cell:
    ...
    for a in range (0,len(categories)):
        for b in range (OGlen,len(ff_df)):
            ...
            pert = HALO(cell,categories_max[a],-1,HaloCellNum)
            if pert!=cell:
