...
PRESCRIPTORS_FILE = '/home/xprize/work/neat-checkpoint-420'
...
RNGfile = int((time.time()-1612339200)/(86400*3))
if RNGfile>7:
    RNGfile=7
readFile = open('/home/xprize/work/CovModel_modfile_1612354700_%d.txt'%RNGfile,'r')
...
def prescribe(start_date_str: str,
    ...
    missing_timer_fin = time.time()
    total_missing_time = missing_timer_fin-missing_timer_start
    print('missing took %f seconds'%total_missing_time)
    ...
    config = neat.Config(neat.DefaultGenome, neat.DefaultReproduction,
                         ...
                         '/home/xprize/work/config-prescriptor')
    ...
    for prescription_idx, prescriptor in enumerate(prescriptors):
        ...
        while action_start_date <= end_date:
            ...
            pred_df = pred_df[pred_df.RegionName != 'Acre']
            pred_df = pred_df[pred_df.RegionName != 'Alagoas']
            pred_df = pred_df[pred_df.RegionName != 'Amazonas']
            pred_df = pred_df[pred_df.RegionName != 'Amapa']
            pred_df = pred_df[pred_df.RegionName != 'Bahia']
            pred_df = pred_df[pred_df.RegionName != 'Ceara']
            pred_df = pred_df[pred_df.RegionName != 'Distrito Federal']
            pred_df = pred_df[pred_df.RegionName != 'Espirito Santo']
            pred_df = pred_df[pred_df.RegionName != 'Goias']
            pred_df = pred_df[pred_df.RegionName != 'Maranhao']
            pred_df = pred_df[pred_df.RegionName != 'Minas Gerais']
            pred_df = pred_df[pred_df.RegionName != 'Mato Grosso do Sul']
            pred_df = pred_df[pred_df.RegionName != 'Mato Grosso']
            pred_df = pred_df[pred_df.RegionName != 'Para']
            pred_df = pred_df[pred_df.RegionName != 'Paraiba']
            pred_df = pred_df[pred_df.RegionName != 'Pernambuco']
            pred_df = pred_df[pred_df.RegionName != 'Piaui']
            pred_df = pred_df[pred_df.RegionName != 'Parana']
            pred_df = pred_df[pred_df.RegionName != 'Rio de Janeiro']
            pred_df = pred_df[pred_df.RegionName != 'Rio Grande do Norte']
            pred_df = pred_df[pred_df.RegionName != 'Rondonia']
            pred_df = pred_df[pred_df.RegionName != 'Roraima']
            pred_df = pred_df[pred_df.RegionName != 'Rio Grande do Sul']
            pred_df = pred_df[pred_df.RegionName != 'Santa Catarina']
            pred_df = pred_df[pred_df.RegionName != 'Sergipe']
            pred_df = pred_df[pred_df.RegionName != 'Sao Paulo']
            pred_df = pred_df[pred_df.RegionName != 'Tocantins']
            pred_df.reset_index(drop=True, inplace=True)
            pred_df['Unnamed: 0'] = pred_df.index
            
            ixx = -1*len(geos)*ACTION_DURATION

            pred_df.to_csv('/home/xprize/work/prescriptions/predChina_%d.csv'%dfCt)
            cost_df.to_csv('/home/xprize/work/prescriptions/costChina_%d.csv'%dfCt)
            print(ixx,"=IXX!")
            ...
            n_cost = 0
            CostCountry = cost_df['CountryName']
            CostRegion = cost_df['CountryName']
            PresCountry = pres_df['CountryName']
            PresRegion = pres_df['RegionName']
            for a in range (0,len(categories)):
                ...
                for b in range (0,ixx*-1):
                    ...
                    try:
                        ...
                        ThisCountry = PresCountry[len(pres_df)+ixx_x]
                        ThisRegion = PresRegion[len(pres_df)+ixx_x]
                        CostGeoIdx = -999
                        for c in range (0,len(CostCountry)):
                            if CostCountry[c]==ThisCountry and CostRegion[c]==ThisRegion:
                                CostGeoIdx = c
                        if CostGeoIdx > -999:
                            n_cost += ((pres_df[categories[a]].iloc[ixx_x])*(cost_df.loc[CostGeoIdx, categories[a]]))
                        else:
                            n_cost += (pres_df[categories[a]].iloc[ixx_x])
                    except:
                        ProceedHALO = False
            ...
            if ProceedHALO == True:
                ...
                for a in range (0,len(categories)):
                    for b in range (len(pert_df)+ixx,len(pert_df)):
                        ...
                        if pert!=cell:
                            ...
                            ll = (b+BaseModIdx)-((b+BaseModIdx)%ACTION_DURATION)
                            ul = ll+ACTION_DURATION
                            for cc in range (ll,ul):
                ...
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Acre']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Alagoas']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Amazonas']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Amapa']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Bahia']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Ceara']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Distrito Federal']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Espirito Santo']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Goias']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Maranhao']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Minas Gerais']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Mato Grosso do Sul']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Mato Grosso']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Para']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Paraiba']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Pernambuco']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Piaui']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Parana']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Rio de Janeiro']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Rio Grande do Norte']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Rondonia']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Roraima']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Rio Grande do Sul']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Santa Catarina']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Sergipe']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Sao Paulo']
                pertpred_df = pertpred_df[pertpred_df.RegionName != 'Tocantins']
                pertpred_df.reset_index(drop=True, inplace=True)
                pertpred_df['Unnamed: 0'] = pertpred_df.index
                ...
                n_cost = 0
                PertCountry = pert_df['CountryName']
                PertRegion = pert_df['RegionName']
                for a in range (0,len(categories)):
                    ...
                    for b in range (0,ixx*-1):
                        ...
                        ThisCountry = PertCountry[len(pert_df)+ixx_x]
                        ThisRegion = PertRegion[len(pert_df)+ixx_x]
                        CostGeoIdx = -999
                        for c in range (0,len(CostCountry)):
                            if CostCountry[c]==ThisCountry and CostRegion[c]==ThisRegion:
                                CostGeoIdx = c
                        if CostGeoIdx > -999:
                            n_cost += ((pert_df[categories[a]].iloc[ixx_x])*(cost_df.loc[CostGeoIdx, categories[a]]))
                        else:
                            n_cost += (pert_df[categories[a]].iloc[ixx_x])
                ...
                for a in range (0,len(categories)):
                    for b in range (0,ixx*-1):
                        pert,word,halo_t = HALO(2,4,HaloCallNum,HaloCellNum,a)
                        HaloCellNum += 2
                        ult_words.append(word)
                        ult_halotime.append(halo_t)
                HaloCallNum += 1
    ...
    prescription_df = pd.concat(prescription_dfs,ignore_index=True)
    ...
    prescription_df.to_csv('/home/xprize/work/unmodified_pres.csv')
    ...
    print(len(prescription_df),len(ult_words),"should be 12 times")
    prescription_df['C1_words'] = ult_words[0:len(prescription_df)]
    prescription_df['C2_words'] = ult_words[len(prescription_df):2*len(prescription_df)]
    prescription_df['C3_words'] = ult_words[2*len(prescription_df):3*len(prescription_df)]
    prescription_df['C4_words'] = ult_words[3*len(prescription_df):4*len(prescription_df)]
    prescription_df['C5_words'] = ult_words[4*len(prescription_df):5*len(prescription_df)]
    prescription_df['C6_words'] = ult_words[5*len(prescription_df):6*len(prescription_df)]
    prescription_df['C7_words'] = ult_words[6*len(prescription_df):7*len(prescription_df)]
    prescription_df['C8_words'] = ult_words[7*len(prescription_df):8*len(prescription_df)]
    prescription_df['H1_words'] = ult_words[8*len(prescription_df):9*len(prescription_df)]
    prescription_df['H2_words'] = ult_words[9*len(prescription_df):10*len(prescription_df)]
    prescription_df['H3_words'] = ult_words[10*len(prescription_df):11*len(prescription_df)]
    prescription_df['H6_words'] = ult_words[11*len(prescription_df):12*len(prescription_df)]
    
    prescription_df['C1_time'] = ult_halotime[0:len(prescription_df)]
    prescription_df['C2_time'] = ult_halotime[len(prescription_df):2*len(prescription_df)]
    prescription_df['C3_time'] = ult_halotime[2*len(prescription_df):3*len(prescription_df)]
    prescription_df['C4_time'] = ult_halotime[3*len(prescription_df):4*len(prescription_df)]
    prescription_df['C5_time'] = ult_halotime[4*len(prescription_df):5*len(prescription_df)]
    prescription_df['C6_time'] = ult_halotime[5*len(prescription_df):6*len(prescription_df)]
    prescription_df['C7_time'] = ult_halotime[6*len(prescription_df):7*len(prescription_df)]
    prescription_df['C8_time'] = ult_halotime[7*len(prescription_df):8*len(prescription_df)]
    prescription_df['H1_time'] = ult_halotime[8*len(prescription_df):9*len(prescription_df)]
    prescription_df['H2_time'] = ult_halotime[9*len(prescription_df):10*len(prescription_df)]
    prescription_df['H3_time'] = ult_halotime[10*len(prescription_df):11*len(prescription_df)]
    prescription_df['H6_time'] = ult_halotime[11*len(prescription_df):12*len(prescription_df)]
    
    w_p = 1/196608
    w_q = 1-w_p
    w_n = len(prescription_df)*12
    
    
    if total_missing_time>=300:
        EvalLOidx = 9
    else:
        EvalLOidx = 3
    
    
    ult_words_eval = ult_words[0:int(len(ult_words)/EvalLOidx)]
    ...
    IncTopWords = False
    if len(sCMW)>len(prescription_df):
        prescription_df['TopWords'] = sCMW[0:len(prescription_df)]
        prescription_df['TopWords_Zscore'] = sZ[0:len(prescription_df)]
        IncTopWords = True

    
    #categories = ['C1_School closing','C2_Workplace closing','C3_Cancel public events','C4_Restrictions on gatherings','C5_Close public transport','C6_Stay at home requirements','C7_Restrictions on internal movement','C8_International travel controls','H1_Public information campaigns','H2_Testing policy','H3_Contact tracing','H6_Facial Coverings']
    if IncTopWords==True:
        prescription_df = prescription_df[['CountryName', 'RegionName', 'Date', 'C1_School closing', 'C1_words', 'C1_time', 'C2_Workplace closing', 'C2_words', 'C2_time', 'C3_Cancel public events', 'C3_words', 'C3_time', 'C4_Restrictions on gatherings', 'C4_words', 'C4_time', 'C5_Close public transport', 'C5_words', 'C5_time', 'C6_Stay at home requirements', 'C6_words', 'C6_time', 'C7_Restrictions on internal movement', 'C7_words', 'C7_time', 'C8_International travel controls', 'C8_words', 'C8_time', 'H1_Public information campaigns', 'H1_words', 'H1_time', 'H2_Testing policy', 'H2_words', 'H2_time', 'H3_Contact tracing', 'H3_words', 'H3_time', 'H6_Facial Coverings', 'H6_words', 'H6_time', 'PrescriptionIndex', 'TopWords', 'TopWords_Zscore']]
    else:
        prescription_df = prescription_df[['CountryName', 'RegionName', 'Date', 'C1_School closing', 'C1_words', 'C1_time', 'C2_Workplace closing', 'C2_words', 'C2_time', 'C3_Cancel public events', 'C3_words', 'C3_time', 'C4_Restrictions on gatherings', 'C4_words', 'C4_time', 'C5_Close public transport', 'C5_words', 'C5_time', 'C6_Stay at home requirements', 'C6_words', 'C6_time', 'C7_Restrictions on internal movement', 'C7_words', 'C7_time', 'C8_International travel controls', 'C8_words', 'C8_time', 'H1_Public information campaigns', 'H1_words', 'H1_time', 'H2_Testing policy', 'H2_words', 'H2_time', 'H3_Contact tracing', 'H3_words', 'H3_time', 'H6_Facial Coverings', 'H6_words', 'H6_time', 'PrescriptionIndex']]
